...
class NotaCredito extends Model
{
	 ...
	 */
	public function scopelistarNota($query, $sucursal, $fecinicio, $fecfin, $tipodocumento, $numero, $proveedor)
	{
		return $query->join('person', 'person.id', '=', 'notacredito.persona_id')
			->join('person as responsable', 'responsable.id', '=', 'notacredito.responsable_id')
			...
			->select('notacredito.*', DB::raw('concat(person.apellidopaterno,\' \',person.apellidomaterno,\' \',person.nombres) as cliente'), DB::raw('responsable.nombres as responsable2'))
			->orderBy('fecha', 'ASC');
	}
